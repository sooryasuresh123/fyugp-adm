...
from django.contrib.auth.decorators import login_required
from django.shortcuts import render
from .models import HODProfile  # Import your HODProfile model
...
def hod_dashboard(request):
    ...
    try:
        ...
        return render(request, 'error.html', {'message': 'HOD profile not found.'})
...
from django.shortcuts import render
from .models import Student, TransferCertificate  # Assuming TC stored in another model
from .forms import StudentAdvancedFilterForm

def advanced_filter_students(request):
    form = StudentAdvancedFilterForm(request.GET or None)
    students = Student.objects.all()

    if form.is_valid():
        data = form.cleaned_data
        if data['program']:
            students = students.filter(program=data['program'])
        if data['stud_adm_no']:
            students = students.filter(stud_adm_no__icontains=data['stud_adm_no'])
        if data['year_of_admission']:
            students = students.filter(year_of_admission=data['year_of_admission'])

        if data['tc']:
            if data['tc'] == 'include':
                students = students.filter(transfercertificate__isnull=False)
            elif data['tc'] == 'exclude':
                students = students.filter(transfercertificate__isnull=True)

        if data['egrantz']:
            if data['egrantz'] == 'include':
                students = students.filter(egrantz=True)
            elif data['egrantz'] == 'exclude':
                students = students.filter(egrantz=False)

        if data['caste']:
            students = students.filter(caste=data['caste'])
        if data['religion']:
            students = students.filter(religion=data['religion'])
        if data['pathway']:
            students = students.filter(pathway=data['pathway'])
        if data['quota']:
            students = students.filter(quota=data['quota'])
        if data['board']:
            students = students.filter(board=data['board'])
        if data['gender']:
            students = students.filter(gender=data['gender'])
        if data['language']:
            students = students.filter(language=data['language'])

    return render(request, 'advanced_filter.html', {'form': form, 'students': students})
